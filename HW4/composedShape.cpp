...
ComposedShape ComposedShape::operator +=(const polygon& obj)
{
	vec.push_back(obj);
}
...
double ComposedShape::shapeElem::perimeter()
{
	...
	{
		Rectangle* Temp = new Rectangle;
		Temp = static_cast<Rectangle*>(shape);
		return 2*(Temp->getWidth()+Temp->getHeight());
	}
	if(q==shapes::Circle)
	{
		Circle* Temp = new Circle;
		Temp = static_cast<Circle*>(shape);
		return PI*2*(Temp->getRadius());
	}
	if(q==shapes::Triangle)
	{
		Triangle* Temp = new Triangle;
		Temp = static_cast<Triangle*>(shape);	
		return 3*(Temp->getSide());
	}
}
...
double ComposedShape::shapeElem::area()
{
	...
	if(q==shapes::Circle)
	{
		Circle* Temp = new Circle;
		Temp = static_cast<Circle*>(shape);
		return PI*(Temp->getRadius())*(Temp->getRadius());
	}
	if(q==shapes::Triangle)
	{
		Triangle* Temp = new Triangle;
		Temp = static_cast<Triangle*>(shape);	
		return (Temp->getSide())*(Temp->getSide())*pow(3,0.5)/4;
	}
}
